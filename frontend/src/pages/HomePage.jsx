...
function KpiDetailModal({ tile, range, onClose }) {
  ...
  useEffect(() => {
    ...
    api
      .dashboardDetails(tile.key, range)
      .then((d) => {
        if (alive) {
          setState({
            loading: false,
...
function FunnelBars({ funnel }) {
  ...
  if (stages.length === 0) {
    return (
      ...
      >
        No tagged contacts yet. Tag contacts to see your
        pipeline here.
      </div>
    );
  }
...
export default function HomePage({
  ...
  const [detailTile, setDetailTile] =
    useState(null);
